...
const TourItem = React.memo(props => {
  ...
  return (
    <div className="tour">
      <img src={TourImg} alt="Tour" className="tour__img" />
      <IconContext.Provider value={{ className: 'tour__like' }}>
        {isLiked ? (
          <IoIosHeart onClick={() => addToWishlist(tour._id)} />
        ) : (
          <IoIosHeartEmpty onClick={() => addToWishlist(tour._id)} />
        )}
      ...
      <Button type="success">Details</Button>
    </div>
  );
});
